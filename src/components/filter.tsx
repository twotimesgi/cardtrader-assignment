...
export const Filter = ({ filter, index }: FilterProps) => {
  ...
  const [rangeValue, setRangeValue] = useState<number[]>(() =>
    filter.attributeType === AttributeType.NUMBER
      ? getCurrentRangeValue(searchParams, attributeKey) ||
        getRangeDefaultValue(filter)
      : []
  );
  ...
  if (filter.attributeType === AttributeType.NUMBER &&
    ...
    !filter.possibleValues.every((value) => !isNaN(Number(value))))) return <></>;
  ...
  if (filter.attributeType === AttributeType.BOOLEAN) {
    ...
    return (
      <div className="flex justify-between items-center border-b">
        <span className="py-4 text-sm font-medium whitespace-nowrap">
      ...
      <AccordionContent>
        <div className="flex flex-wrap gap-2 mt-2 min-w-[250px] overflow-hidden">
          {filter.attributeType === AttributeType.STRING &&
            filter.possibleValues.map((value) => {
              const selectedValues =
                searchParams.get(attributeKey)?.split(",") || [];
          ...
          {filter.attributeType === AttributeType.NUMBER &&
            ...
            filter.possibleValues.every((value) => !isNaN(Number(value))) && (
              <div className="w-full pt-8 pb-2 px-2">
                <DualRangeSlider
                  ...
                  min={Math.min(
                    ...filter.possibleValues.map((v) => parseFloat(v))
                  )}
                  max={Math.max(
                    ...filter.possibleValues.map((v) => parseFloat(v))
                  )}
